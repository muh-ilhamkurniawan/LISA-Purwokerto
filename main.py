...
qa_pairs = {
    ...
    "lanjutan": "Adakah hal lain yang bisa Lisa bantu lagi? Silakan ketik 'Menu' untuk kembali ke menu utama atau ketik 'Selesai' untuk mengakhiri percakapan ini yaa 😊",
    ...
    "selesai": "Terimakasih sudah menggunakan layanan LISA. Jika ada yang mau ditanyakan silahkan ketik 'Halo'😊"
}
...
def generate_answer():
    ...
    if user_message == "selesai":
        next_node = "selesai"  # Mengatur kembali ke awal
        current_sequence = ""  # Mengosongkan sequence
    else:
        # Update node berdasarkan input
        # if user_message.isdigit() and user_message not in ["halo", "lanjutan", "menu"]:
        if user_message.isdigit() and user_message not in ["halo", "lanjutan", "menu"]:
            if current_sequence == "":
                current_sequence = user_message
                next_node = user_message
            else :
                next_node = f"{current_sequence}.{user_message}"
        else:
            next_node = user_message

    # Cari jawaban berdasarkan pertanyaan yang telah didefinisikan
    message_bot = qa_pairs.get(next_node, "Maaf, saya tidak mengerti pertanyaan Anda.")
    
    # Tambahkan pesan pengguna ke riwayat
    st.session_state.history.append({"message": user_message, "is_user": True})
    # Tambahkan jawaban chatbot ke riwayat
    # st.session_state.history.append({"message": (f"current_sequence = {current_sequence}"), "is_user": False})
    # st.session_state.history.append({"message": (f"next_node = {next_node}"), "is_user": False})
    st.session_state.history.append({"message": message_bot, "is_user": False})
    
    # Tambahkan pesan lanjutan jika diperlukan
    if next_node in ["1.1", "1.2", "1.3", "1.4", "1.5", "2.1", "2.2", "2.3", "3", "4.1", "4.2", "4.3", "5.1", "5.2"]:
...
input_area.text_input("Talk to the bot", key="input_text", on_change=generate_answer)
